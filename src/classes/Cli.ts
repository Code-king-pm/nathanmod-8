...
class Cli {
  ...
  findVehicleToTow(truck:Truck): void {
    inquirer
      ...
      .then((answers) => {let selectedVehicle = answers.vehicleToTow;
      
        // Check if the truck can tow the selected vehicle based on weight
        if (selectedVehicle.weight <= truck.towingCapacity) {
          selectedVehicle += truck.towingCapacity  //todo: Add vehicle to the truck's towed vehicles----------------------------------------------
          console.log(
            `Successfully attached ${selectedVehicle.make} ${selectedVehicle.model} to the truck!`
          );
        } else {
          console.log(
            `The selected vehicle (${selectedVehicle.make} ${selectedVehicle.model}) is too heavy for this truck to tow.`
          );
        }
      ...
      .then((answers) => {
        ...
        // TODO: add statements to perform the tow action only if the selected vehicle is a truck. Call the findVehicleToTow method to find a vehicle to tow and pass the selected truck as an argument. After calling the findVehicleToTow method, you will need to return to avoid instantly calling the performActions method again since findVehicleToTow is asynchronous.
          ...
          if (selectedVehicle instanceof Truck) {
            // Call findVehicleToTow and pass the truck object
            this.findVehicleToTow(selectedVehicle);
            return; // Avoid instantly calling performActions again
          } else {
            // Log an error message for non-truck vehicles
            console.log('Towing can only be performed by a truck!');
            // Restart the action prompt
            this.performActions();
          }
        }
